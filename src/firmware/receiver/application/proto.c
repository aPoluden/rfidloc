...
wifi_tx(unsigned char power, unsigned char channel) {
    ...
    /* update crc */
    g_Beacon.pkt.crc = swapshort(env_crc16(g_Beacon.byte, sizeof (g_Beacon) - sizeof (u_int16_t)));
...
void
wifi_tx_reader_command(unsigned int reader_id, unsigned char opcode, unsigned int data0, unsigned int data1) {
                ...
                if (swapshort(g_Beacon.pkt.crc) == crc) {
                    ...
                    switch (g_Beacon.pkt.proto) {
                        ...
                        case RFBPROTO_BEACONTRACKER:
                            ...
                            if ((g_BeaconTx.pkt.proto == RFBPROTO_PROXTRACKER) && (g_BeaconTx.pkt.p.tracker.oid_last_seen == g_Beacon.pkt.oid)) {
